...
<canvas height='800' width='1200' id='example'>Обновите браузер</canvas>
<script>
	var razm = 35;
	var step;
	var potok = 600;
	...
	var ctxm = example.getContext('2d');
	var speed = 5;
	var uscor = 10;
	...
	function gowall(ofset) {
		...
		ctx.clearRect(0, 50, example.width, razm);
		ctx.clearRect(0, 710, example.width, razm);
		if (ofset === 0) {
			ofset = razm*2*speed;
			brick();
		}
	...
	switch (step){
			...
			case 65://A
				uscor = 10;
				position[0].y-=speed;
				if (position[0].y < 0) position[0].y = 0;
				break;
			case 38://вверх
			case 87://W
				position[0].x-=speed;
				if (position[0].x < 0) position[0].x = 0;
				break;
			case 39://право
			case 68://D
				uscor = 5
				position[0].y+=speed;
				if (position[0].y > 700) position[0].y = 700;
				break;
			case 40://вниз
			case 83://S
				position[0].x+=speed;
				if (position[0].x > 700) position[0].x = 700;
				break;
			default:
				break;
		}
	...
	function brick(){
		...
		if (position.length>1 && position[1].y < -machine.w)
			position.splice(1, 1)
		x=Math.floor(Math.random() * (640 - 350) + 350);
